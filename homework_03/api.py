...
class EmailField(CharField):
    ...
    def check_requirements(self, value: str):
        ...
        pattern = re.compile(
            r'([A-Za-z0-9]+[.-_])*[A-Za-z0-9]+@[A-Za-z0-9-]+(\.[A-Z|a-z]{2,})+', re.IGNORECASE)
...
class PhoneField(Field):
    ...
    def check_value_type(self, value):
        ...
        if not isinstance(value, (str, int)):
            raise TypeError(
                'Переданное значение не является строкой или целым числом')
        return str(value)
...
class BirthDayField(DateField):
    ...
    def check_requirements(self, value):
        ...
        if (today - value).days / 365.25 > MAX_AGE:
            raise ValueError(f'Превышено максимальное значение возраста, \
...
class GenderField(Field):
    ...
    def check_value_type(self, value: int):
        if value is not None and not isinstance(value, int):
            raise TypeError(
                'Переданное значение не является целым числом. Допустимые значения: 0, 1, 2')
        return value

    def check_requirements(self, value: int):
        if value not in GENDERS:
            raise ValueError(
                'Переданное значение пола не является допустимым.')
    ...
    def check_value_type(self, value: list):
        if value is not None:
            if not isinstance(value, list):
                ...
                raise ValueError(
                    'В переданном списке присутствует значения не являющиеся целыми числами.')
        return value
    ...
    def validate(self):
        ...
        if not self.fields_errors:
            if not (self.phone and self.email) and not (self.first_name and self.last_name) \
                    and not (self.gender is not None and self.birthday):
...
class ClientsInterestsHandler:
    ...
    def process_request(self, request, context, store):
        clients_result = ClientsInterestsRequest(request.arguments)
        if not clients_result.is_valid():
            return clients_result.errors, INVALID_REQUEST
        context["nclients"] = len(clients_result.client_ids)
        response_body = {cid: scoring.get_interests(
            store, cid) for cid in clients_result.client_ids}
...
def check_auth(request):
    if request.is_admin:
        ...
        digest = hashlib.sha512(
            bytes(request.account + request.login + SALT, "utf-8")).hexdigest()
